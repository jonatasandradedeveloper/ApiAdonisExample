# Ana Karina — ações

Lista operacional. Marque, datar, mover para o histórico quando concluir.

## Urgente (antes de meados de setembro)

### People Lead

- [ ] Agendar 1:1 #2 com quadro trimestre / semestre / ano na pauta
- [ ] Pegar e-mail da Paula e do Rafael, se ainda não chegou
- [ ] Revisar rascunho da autoavaliação **antes** da submissão
- [ ] Combinar expectativa de cargo (consolidar vs. conversa de Analyst)

### Ana Karina

- [ ] Recortar prioridades no Workday (3 frentes)
- [ ] Escrever 1 case de Copilot (problema → ação → efeito)
- [ ] Listar 2–3 entregas do app para a autoavaliação
- [ ] Café com o Rafael: percepção de performance
- [ ] Feedback de um par Android e, se possível, um iOS
- [ ] Mini CV + Copilot primário + inglês C1+ visível
- [ ] Compliance conferido
- [ ] Voluntariado: fato ou honestidade de que não andou

## Até o feedback de novembro

- [ ] PL: avaliação no sistema
- [ ] PL: insumos para calibração (`04-insumos-ciclo.md`)
- [ ] Decidir se o case de Copilot vai para a comunidade da prática (opcional, alto retorno)
- [ ] Trilha FY27: certificação IA + Swift, com datas depois da faculdade / do ciclo

## Backlog de carreira (não é setembro)

- [ ] Certificação foundations de IA
- [ ] Swift com aplicação (não só playlist)
- [ ] Mentoria pontual de arquitetura mobile
- [ ] Conversar próximo desafio quando o app atual não for mais a melhor sala de aula

## Log de ações

| Data | Ação | Quem | Resultado |
|---|---|---|---|
| 2026-07-28 | 1:1 #1 | ambos | Cadência mensal; pedido de plano tipo Paula |
| 2026-08-24 | Dossiê inicial do PL | PL | Parecer: recortar 7 prioridades; Copilot é o ouro |
